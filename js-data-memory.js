...
function MemoryAdapter(opts) {
  ...
  this._updateAll = function (resource, attrs, params, options) {
    ...
    return this.findAll(resource, params, options).then(function (items) {
      ...
      items.forEach(function (item) {
        tasks.push(self.update(resource, item[resource.idAttribute], attrs, options));
      });
      return Promise.all(tasks)
      .then(function(results) {
        return [results, {}];
  ...
  this._destroyAll = function (resource, params, options) {
    ...
    return this.findAll(resource, params, options).then(function (items) {
      var tasks = [];
      items.forEach(function (item) {
        tasks.push(self.destroy(resource, item[resource.idAttribute], options));
      });
      return Promise.all(tasks)
      .then(function(results) {
        return [results, {}];
  ...
  this.loadBelongsTo = function (mapper, def, records, __opts) {
    ...
    if (mapper.relations[def.type][def.relation].get) {
      ...
      var p = [];
      records.forEach(function (record) {
        p.push(mapper.relations[def.type][def.relation].get({}, {}, record, __opts)
      ...
      if (singular) {
        ...
        var keys = [];
        records.forEach(function (record) {
          var key = self.makeBelongsToForeignKey(mapper, def, record);
      ...
      if (records.length == 1) {
        return self.findAll(relatedMapper, { 
          where: defineProperty({}, relatedMapper.idAttribute, { 
            'in': self.makeHasManyLocalKeys(mapper, def, records[0]) 
          }) 
        }, __opts).then(function (relatedItems) { 
          def.setLocalField(records[0], relatedItems);
        });
      } else {
        var localKeys = [];
        records.forEach(function (record) {
      ...
      if (records.length == 1) {
        return self.findAll(def.getRelation(), {
          where: defineProperty({}, def.foreignKeys, {
            'contains' : self.makeHasManyForeignKeys(mapper, def, records[0])
          })
        }, __opts).then(function (relatedItems) {
          def.setLocalField(records[0], relatedItems);
        });
        ...
        var p = [];
        records.forEach(function (record) {
          p.push(self.findAll(relatedMapper, {
            where : defineProperty({}, def.foreignKeys, {
